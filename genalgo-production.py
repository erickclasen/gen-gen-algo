...
def gaussian_mutate(pop,mutation_rate,dither_range=0.1,force=False):
        ''' Allows an element of the population picked at random to be mutated by replacement via a random value.
       ...
       # Mutation, mutate at a 5% rate, pick a random index and mutate that populatin with a random seed.
        if random.random() < mutation_rate or force: # Happens infrequently unless force is used to make it happen.
                ...
                # 07022020 TWEAK - Try to mutate random element. Need to copy this fxn without this line below.
                mean = pop[mut_index]   # Mutate off of the element itself. sum(bounds)/2

                std_dev = abs(mean * dither_range)   # Keep std dev ralatie to the mean to scale it. 
...
def gaussian_mutate_element(pop,element,mutation_rate,dither_range=0.1,force=False):
       ...
       # Mutation, mutate at a 5% rate, pick a random index and mutate that populatin with a random seed.
        if random.random() < mutation_rate or force: # Happens infrequently unless force is used to make it happen.
                mean = pop[element]   # Mutate off of the element itself. sum(bounds)/2
                std_dev = abs(mean * dither_range)
...
def fitness_evaluation(pop,opt_for_min,fitness_offspring,fitness_ref,C_value,sample_pop):
        ...
        if opt_for_min and fitness_offspring < fitness_ref: # Minimize the error in this case. Others cases may involve mazimizing op$
                ...
                if fitness_offspring < best_fitness:
                        ...
                        print("Offspring -> pop[0] and store pop[0] -> pop["+str(sample_pop)+"] , Update Best Fitness: ",best_fitness)
                        pop[sample_pop] = pop[0] # * C_value/2  # Demote and breed the previous best fit with the new best, overwritting the less fit reference.
                ...
                if fitness_offspring > best_fitness:
                        ...
                        (pop[0],pop[sample_pop]) = (pop[sample_pop],pop[0]) # Swap vars using a tuple trick.
                else:
...
def ga(fxn_to_pass,pop,bounds,opt_for_min,mutation_rate):
        ...
        # Take a checksum of the pop before any modification to see if it changes later in this fxn.
        chksum = sum(pop)        
       ...
       # Mutation, mutate at a 5% rate, pick a random index and mutate that populatin with a random seed.
        ...
        # Testing out a Gaussian mutation to apply Dither when diversity is low.
        if abs(diversity) < 0.25:
                print("Diversity is low... mutate.")
                #gaussian_mutate(pop,mutation_rate,force=True)
                mutate(pop,bounds,1)
        elif chksum == sum(pop):   # Stagnant population.
                print("Population is stagnant, apply dither to random element at mutation rate: ",mutation_rate)
                gaussian_mutate(pop,mutation_rate)
...
def run_algo(fxn_to_pass,pop=[],bounds=(0,1),opt_for_min=True,mutation_rate=0.0,pop_init_len=10,loops=10,optimization_threshold=0.0,warmup_loops=10,smoothing_const=0.1,run_ga_code=True):
        ...
        for n in range(0,loops):
                ...
                alpha_element = pop[0] #Save off he best, to compare below.
                ...
                elif pop[0] == alpha_element:
                        print("No change in pop[0]. Timeout Count: ",count)
